...
import { Container, Header, WrapperForm, Modules, ActiveModule, NumberModule } from './style'
      ...
      <WrapperForm>
        <div>
          {module.questions.map((data: any, key: number) => (
            <QuestionOfPoll title={data.question} options={data.options} moduleId={module.id} />
          ))}
        </div>
        <Modules>
          {dataPoll.map((data: any, key: number) => (
            ...
            >
              <NumberModule isActive={keyModule === key}>{key + 1}</NumberModule>
              {data.module}
            </ActiveModule>
          ))}
        </Modules>
